...
def get_dataloader(
    dataset, pad_token_id, batch_size, shuffle=True, num_workers=1, **dataloader_args
):
    def collate_fn(batch):
        ...
        question_ids = [
            ids + [pad_token_id] * (max_q_len - len(ids)) for ids in question_ids
        ]
        answer_ids = [
            ids + [pad_token_id] * (max_a_len - len(ids)) for ids in answer_ids
        ]
        q_attn_mask = [mask + [0] * (max_q_len - len(mask)) for mask in q_attn_mask]
        a_attn_mask = [mask + [0] * (max_a_len - len(mask)) for mask in a_attn_mask]
        # Convert to tensors
